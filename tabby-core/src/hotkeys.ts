    ...
    hotkeys: HotkeyDescription[] = [
        ...
        },
        {
            ...
            name: this.translate.instant(
                'Turn current tab\'s panes into separate tabs'
            ),
        },
        {
            id: 'combine-tabs',
            name: this.translate.instant(
                'Combine all tabs into the current tab'
            ),
        },
        {
            id: 'tab-1',
    ...
    async provide (): Promise<HotkeyDescription[]> {
        ...
        return [
            ...
            ...profiles.map((profile) => ({
                id: `profile.${AppHotkeyProvider.getProfileHotkeyName(
                    profile
                )}`,
                name: this.translate.instant('New tab: {profile}', {
                    profile: profile.name,
                }),
            })),
            ...this.profilesService.getProviders().map((provider) => ({
                id: `profile-selectors.${provider.id}`,
                name: this.translate.instant('Show {type} profile selector', {
                    type: provider.name,
                }),
            })),
        ]
